...
public enum Template {
    BankAccount("银行账号",
            ...
            List.of("bank.account.base", "bank.account.date.end", "bank.account.id.length")),
    BankBill("银行账单明细",
            "转出账号, 转账人姓名, 收款账号, 收款人姓名, 转账金额, 交易类型, 转账渠道, 转账方式, 转账时间, 转账附言, 手续费",
            List.of("bank.bill.base", "bank.bill.charge", "bank.bill.date.end", "bank.bill.date.format")),
    Bus("汽车出行",
            "姓名, 性别, 身份证号, 出行日期, 车次, 出发汽车站, 发车时间, 座位号, 出发城市, 到达城市",
            List.of("travel.date.end", "travel.date.format")),
    CDC("CDC",
    ...
    People("人口",
            ...
            List.of("people.birthday.begin", "people.birthday.end")),
    PhoneCall("话单数据",
            "主叫人姓名, 主叫人身份证, 主叫人电话号码, 被叫人姓名, 被叫人身份证, 被叫人电话号码, 通话日期, 开始时间, 通话时常",
            List.of("phone.date.end", "phone.date.format", "phone.duration.base")),
    S_CORP_RCV("外汇局RCV数据",
            "S_CORP_RCV;RPTNO, 企业组织机构代码, RCV_DATE, SAFECODE, 收汇美元金额, 交易代码",
            List.of("safe.base", "safe.date.begin.year", "safe.date.end.year", "safe.date.format", "safe.prefix")),
    S_EXPF("外汇局EXPF数据",
    ...
    Template(String name, String desc, List<String> configs) {
        this.name = name;
        this.desc = desc;
